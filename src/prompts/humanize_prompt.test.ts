...
describe("HUMANIZE_REVIEW_SYSTEM_PROMPT", () => {
  ...
  it("names every tell with the identical user-facing title", () => {
  ...
  it("gives every tell its slug so findings can be traced back", () => {
    for (const tell of listAllTells()) {
      expect(
        HUMANIZE_REVIEW_SYSTEM_PROMPT,
  ...
  // The panel lists this taxonomy and the prompt reports against it. Reading
  // both from the same source is what stops a user meeting a finding for a
  // pattern the panel never mentioned.
  it("covers the tells no pattern can catch", () => {
    expect(listAllTells().length).toBe(19);
  });

  // A finding is applied by opening the path and replacing the quoted run
  // literally, so the prompt has to ask for something that can be opened and
  // something that exists as one string.
  it("asks for a path that can be opened and a line that can be matched", () => {
    expect(HUMANIZE_REVIEW_SYSTEM_PROMPT).toMatch(
      /Give one path in \*\*Relevant Files\*\* and nothing else/,
    );
    expect(HUMANIZE_REVIEW_SYSTEM_PROMPT).toMatch(
      /a replacement cannot span a tag/,
    );
  });
